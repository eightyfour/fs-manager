...
//                        editor.setReadOnly(true);
    ...
    events.addServerListener('sendFile', function (obj) {
        ...
        if (obj.fileType === C.FILE_MANAGER.FILE_TYPES.FILE) {
            ...
            tabSessionId = canny.tabManager.addTab({
                onclick : function () {
                    var session = sessionHandler.session[editorSessionId];
                    fc.hideEditors();
                    fc.showInEditor(editorSessionId);
                    canny.tabManager.activeTab(session.tabSessionId);
                },
                onclose : function () {
                    var session = sessionHandler.session[editorSessionId];
                    fc.removeEditor(editorSessionId);
                    canny.tabManager.removeTab(session.tabSessionId);
                },
                text : obj.name,
                id : id
            });
            ...
            canny.tabManager.showTabBar();
        } else {
            canny.tabManager.hideTabBar();
        }
    });
...
canny.add('fileEditor', fileEditor);